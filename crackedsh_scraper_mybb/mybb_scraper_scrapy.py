...
def run_scraper(max_pages=30, top_n=10):
    ...
    print(f"Finding top {top_n} combolists from today with highest numbers in titles...")
    ...
    print(f"\nScraping completed! Found top {top_n} combolists from today.")
...
def print_results():
    ...
    try:
        ...
        print("No results file found. The scraper may not have found any threads from today.")


def main():
    parser = argparse.ArgumentParser(description='Scrapy-based MyBB Forum Scraper for cracked.sh')
    parser.add_argument('--max-pages', type=int, default=30, help='Maximum number of pages to scrape (default: 30)')
    parser.add_argument('--top-n', type=int, default=10, help='Number of top combolists to return (default: 10)')
    parser.add_argument('--print-results', action='store_true', help='Print results to console after scraping')
